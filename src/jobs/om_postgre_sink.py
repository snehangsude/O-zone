...
def parse_openmeteo_data(data: str) -> Row:
    ...
    uv_index_clear_sky = data['uv_index_clear_sky']
...
def filter_openmeteo_pollution(value: str) -> str | None:
    AQI_THRESHOLD = 110
...
def initialize_openmeteo_env() -> StreamExecutionEnvironment:
...
def configure_openmeteo_source(server: str, earliest: bool = False) -> KafkaSource:
    ...
    kafka_source = (
        KafkaSource.builder()
        .set_topics("openMeteo_pollution_data")
...
def configure_openmeteo_postgre_sink(sql_dml: str, type_info: Types) -> JdbcSink:
...
def configure_openmeteo_kafka_sink(server: str, topic_name: str) -> KafkaSink:
...
def openmeteo_execute() -> None:
    ...
    env = initialize_openmeteo_env()

    # Define source and sinks
    logger.info("Configuring source and sinks")
    kafka_source = configure_openmeteo_source(KAFKA_HOST)
    sql_dml = (
    "INSERT INTO pollutantsOpenMeteo (message_id, date, time, lat, lon, city, aqi, carbon_monoxide, nitrogen_dioxide, ozone, sulphur_dioxide, fine_particles2_5, coarse_particles10, aerosol_optical_depth, dust, uv_index, uv_index_clear_sky) "
    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
)
    ...
    TYPE_INFO = Types.ROW(
        [
            ...
            Types.FLOAT(),   # aerosol_optical_depth
            Types.FLOAT(),   # dust
            Types.FLOAT(),   # uv_index
            Types.FLOAT()    # uv_index_clear_sky
        ]
    )

    jdbc_sink = configure_openmeteo_postgre_sink(sql_dml, TYPE_INFO)

    kafka_sink = configure_openmeteo_kafka_sink(KAFKA_HOST, "openmeteo_high_pollution")
    ...
    transformed_data = data_stream.map(parse_openmeteo_data, output_type=TYPE_INFO)
    alarms_data = data_stream.map(
        filter_openmeteo_pollution, output_type=Types.STRING()
    ...
    env.execute("OpenMeteo: Air Pollution Data")
